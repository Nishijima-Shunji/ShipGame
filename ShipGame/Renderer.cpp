...
void Renderer::Init()
{
	...
	swapChainDesc.BufferDesc.Width = App::GetWidth(); // バッファの幅をウィンドウサイズに合わせる
	swapChainDesc.BufferDesc.Height = App::GetHeight(); // バッファの高さをウィンドウサイズに合わせる
	...
	swapChainDesc.OutputWindow = App::GetWindow(); // スワップチェーンのターゲットウィンドウを設定
	...
	viewport.Width = (FLOAT)App::GetWidth();   // ビューポートの幅
	viewport.Height = (FLOAT)App::GetHeight(); // ビューポートの高さ
...
void Renderer::SetWorldViewProjection2D()
{
	...
	Matrix projection = DirectX::XMMatrixOrthographicOffCenterLH(
		0.0f,
		static_cast<float>(App::GetWidth()),	// ビューボリュームの最小Ｘ
		static_cast<float>(App::GetHeight()),	// ビューボリュームの最小Ｙ
		0.0f,											// ビューボリュームの最大Ｙ
		0.0f,
		1.0f);
